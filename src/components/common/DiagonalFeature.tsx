...
const DiagonalFeatureSection = ({
  ...
  const sections = [
    ...
    { heading, description, imageSrc },
    { heading, description, imageSrc },
  ]; // Number of sections and their contents

  return (
    <div className="w-full md:w-[90%] py-16 mx-auto p-4 bg-white rounded-lg overflow-hidden shadow-md mb-10">
      {sections.map((section, index) => (
        <div key={index} className="relative mb-32">
          <div className={`flex md:gap-32 flex-col-reverse md:flex-row ${index % 2 !== 0 ? 'md:flex-row-reverse' : ''}`}>
            {/* Image container */}
            <div className="relative w-full md:w-[42%] flex justify-center items-center">
              ...
              <div className="relative z-10 w-full h-full overflow-hidden">
                <img
                  ...
                  className="w-full h-full object-cover md:w-[413px] md:h-[482px]"
                />
              </div>
            </div>
            {/* Text container */}
