import sunGlasses from './../../assets/careerImages/face-sunglasses 1.png'
interface ValueCardType {
  icon: string,
  title: string,
  description: string
}
const ValueCard = ({ icon: Icon, title, description }: ValueCardType) => (
  <div className="bg-[#FFFFFF] w-[240px] h-[240px] -top-2  p-6 rounded-[24px] shadow-sm hover:shadow-md ">
    <div className='flex justify-between'>
      <div className="flex items-center mb-4">
        <h3><span className='font-normal text-[14] font-inter'>{title.split(" ")[0]}</span> <br /> <span className='font-normal text-[24px] font-inter'>
          {title.split(" ")[1]} {title.split(" ")[2]} </span> </h3>
      </div>
      <div className="font-inter font-normal text-base leading-7 text-[#838696]">
        <img src={Icon} className="w-5 h-5 text-purple-600" />
      </div>
    </div>
